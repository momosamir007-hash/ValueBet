...
class Disp:
    ...
    def backtest(r: dict):
        ...
        print(box(f" X2 (Away/Draw): {C.yellow(f'{r.get(\"dc_x2_acc\",0):.1f}%')} ({r.get('dc_x2_n',0)} matches)"))
        print(box(f" 12 (No Draw): {C.cyan(f'{r.get(\"dc_12_acc\",0):.1f}%')} ({r.get('dc_12_n',0)} matches)"))
        print(f" {C.blue('├'+'─'*w+'┤')}")
